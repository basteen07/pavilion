...
// --- Secondary SMTP Transporter ---
let transporter2 = null;
if (EMAIL_ENABLED && process.env.SMTP2_USER) {
...
// --- Helper: Get transporter + from address by senderKey ---
function getTransporterBySender(senderKey) {
  if (senderKey === 'secondary' && transporter2) {
    return {
      transport: transporter2,
      from: `"${process.env.SMTP2_FROM_NAME || 'Pavilion Sports'}" <${process.env.SMTP2_FROM || process.env.SMTP2_USER}>`,
    };
  }
  // Default to primary
  return {
...
// --- Get available senders for the frontend ---
export function getAvailableSenders() {
  const senders = [];
  if (process.env.SMTP_USER) {
    senders.push({
      key: 'primary',
      email: process.env.SMTP_FROM || process.env.SMTP_USER,
      name: process.env.SMTP_FROM_NAME || 'Pavilion Sports',
      label: `${process.env.SMTP_FROM_NAME || 'Pavilion Sports'} (${process.env.SMTP_FROM || process.env.SMTP_USER})`,
    });
  }
  if (process.env.SMTP2_USER) {
    senders.push({
      key: 'secondary',
      email: process.env.SMTP2_FROM || process.env.SMTP2_USER,
      name: process.env.SMTP2_FROM_NAME || 'Pavilion Sports',
      label: `${process.env.SMTP2_FROM_NAME || 'Pavilion Sports'} (${process.env.SMTP2_FROM || process.env.SMTP2_USER})`,
    });
  }
  return senders;
}

// Core email sending function (supports senderKey for multi-SMTP)
export async function sendEmail({ to, subject, html, text, attachments, senderKey }) {
  const { transport, from } = getTransporterBySender(senderKey);

  if (!EMAIL_ENABLED || !transport) {
    // Mock mode - log emails instead of sending
    console.log('\n=== MOCK EMAIL SERVICE ===');
    console.log('Sender:', senderKey || 'primary');
  ...
  try {
    ...
    console.log(`[Email] Sent successfully via ${senderKey || 'primary'}:`, info.messageId);
    ...
    console.error(`[Email] Failed to send via ${senderKey || 'primary'}:`, error.message);
    return {
      success: false,
      error: error.message,
...
export async function sendQuotationEmail(quotation, customerEmail, senderKey, message = null) {
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="padding: 30px; background: #f8f9fa;">
          ...
          ${message ? `
            <div style="background: #ffffff; border-left: 4px solid #3b82f6; padding: 15px; margin: 20px 0; color: #374151;">
              ${message.replace(/\n/g, '<br>')}
            </div>
          ` : ''}
