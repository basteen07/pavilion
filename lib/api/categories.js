...
// GET /api/tags?subCategoryId=...
// GET /api/tags?subCategoryId=...&slug=...
export async function getTags(params) {
    try {
        ...
        // Handle both old signature (just ID) and new object params
        const subCategoryId = typeof params === 'object' ? params.subCategoryId : params;
        const slug = typeof params === 'object' ? params.slug : null;

        if (subCategoryId) {
            queryStr += ` AND sub_category_id = $${paramCount++}`;
        ...
        if (slug) {
            // Slugify checks
            queryStr += ` AND (
                LOWER(REPLACE(REPLACE(name, ' ', '-'),'--','-')) = $${paramCount} 
                OR name ILIKE $${paramCount}
            )`;
            // exact match attempts logic or just assume slug match
            // actually let's just use exact name match if slug is passed as name? 
            // or generated slug match. Postgres doesn't save slug for tags usually.
            // "english-willow-cricket-bats" -> Name "English Willow Cricket Bats"
            // reverse slugify is hard.
            // Ideally we should save slug in DB.
            // If DB doesn't have slug column, we must match by generated slug
...
export async function deleteTag(id) {
    try {
        // First, unlink the tag from any products that use it
        await query('UPDATE products SET tag_id = NULL WHERE tag_id = $1', [id]);
