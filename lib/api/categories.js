...
export async function getTags(params) {
    try {
        ...
        if (subCategoryId) {
            queryStr += ` AND t.sub_category_id = $${paramCount++}`;
        ...
        if (slug) {
            // Slugify checks
            queryStr += ` AND (
                LOWER(REPLACE(REPLACE(t.name, ' ', '-'),'--','-')) = $${paramCount} 
                OR t.name ILIKE $${paramCount}
            )`;
            queryParams.push(slug);
            paramCount++;
        }

        queryStr += ' ORDER BY t.display_order ASC, t.name ASC';
