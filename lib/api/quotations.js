...
export async function getQuotations(searchParams) {
    try {
        ...
        // Lazy Migration: Add is_detailed column if it doesn't exist
        await query(`ALTER TABLE quotation_items ADD COLUMN IF NOT EXISTS is_detailed BOOLEAN DEFAULT false;`);
...
export async function getQuotationById(id, adminId = null) {
    try {
        ...
        await query(`ALTER TABLE quotation_items ADD COLUMN IF NOT EXISTS slug TEXT;`);
        await query(`ALTER TABLE quotation_items ADD COLUMN IF NOT EXISTS is_detailed BOOLEAN DEFAULT false;`);
        await query(`ALTER TABLE quotation_items ADD COLUMN IF NOT EXISTS short_description TEXT;`);
        await query(`ALTER TABLE quotation_items ADD COLUMN IF NOT EXISTS image_url TEXT;`);
        await query(`ALTER TABLE quotation_items ADD COLUMN IF NOT EXISTS uom TEXT DEFAULT 'Single';`);

        const itemsRes = await query(`
            SELECT qi.*, COALESCE(qi.is_detailed, false) as is_detailed, 
            p.dealer_price as current_dealer_price, p.mrp_price as current_mrp, p.images, 
        ...
        // Log retrieved items
        console.log(`[GetQuotationById] Retrieved items for quote ${id}:`);
        itemsRes.rows.forEach(item => {
            console.log(`  - ${item.product_name}: is_detailed=${item.is_detailed} (type: ${typeof item.is_detailed})`);
        });
        if (adminId) {
            try {
                ...
                await logActivity({
                    ...
                    metadata: {
                        ...
                        customer: quotation.company_name || quotation.customer_email
                    }
                });
            } catch (logErr) {
                console.error('Failed to log quotation view:', logErr);
            }
        }
...
export async function createQuotation(data, adminId = null) {
    try {
        ...
        // Lazy Migration: Add new columns for item metadata
        await query(`ALTER TABLE quotation_items ADD COLUMN IF NOT EXISTS short_description TEXT;`);
        await query(`ALTER TABLE quotation_items ADD COLUMN IF NOT EXISTS image_url TEXT;`);
        await query(`ALTER TABLE quotation_items ADD COLUMN IF NOT EXISTS category_name TEXT;`);
        await query(`ALTER TABLE quotation_items ADD COLUMN IF NOT EXISTS sub_category_name TEXT;`);
        await query(`ALTER TABLE quotation_items ADD COLUMN IF NOT EXISTS brand_name TEXT;`);
        await query(`ALTER TABLE quotation_items ADD COLUMN IF NOT EXISTS uom TEXT DEFAULT 'Single';`);

        // Insert items with full metadata
        for (const item of items) {
            // Explicit boolean conversion that handles strings correctly
            let isDetailedValue = false;
            // Check for truthy values explicitly including string representations
            if (item.is_detailed === true || item.is_detailed === 1 || item.is_detailed === 'true' || item.is_detailed === '1') {
                isDetailedValue = true;
            } else {
                isDetailedValue = false;
            }

            console.log(`[CreateQuotation] Saving item ${item.product_name} | payload.is_detailed: ${item.is_detailed} (${typeof item.is_detailed}) -> saved: ${isDetailedValue}`);

            await query(
                `INSERT INTO quotation_items (
                    quotation_id, product_id, product_name, quantity, unit_price, 
                    total_price, line_total, mrp, dealer_price, discount, slug, is_detailed,
                    short_description, image_url, category_name, sub_category_name, brand_name, uom
                )
...
export async function updateQuotation(id, data, adminId = null) {
    try {
        ...
        // Calculate new total ONLY if items are provided, otherwise keep existing
        let total_amount = data.total_amount;
        if (total_amount === undefined && items) {
            total_amount = items.reduce((sum, item) => sum + (Number(item.unit_price) * Number(item.quantity)), 0);
        }
        ...
        // Replace items ONLY if new items are provided
        if (items) {
            await query(`DELETE FROM quotation_items WHERE quotation_id = $1`, [id]);

            for (const item of items) {
                // Debug log
                console.log(`[UpdateQuotation] Updating item ${item.product_name} | is_detailed payload:`, item.is_detailed);
                ...
                console.log(`[UpdateQuotation] Saving item ${item.product_name} | payload.is_detailed: ${item.is_detailed} (${typeof item.is_detailed}) -> saved: ${isDetailedValue}`);
        ...
        if (adminId) {
            try {
                ...
                await logActivity({
                    ...
                    description: `Updated quotation ${quotationNumber} - Status: ${status || 'unchanged'}, Amount: ₹${quoteResult.rows[0].total_amount}`,
                    metadata: { quotation_number: quotationNumber, status, amount: quoteResult.rows[0].total_amount, items_count: items?.length || 0 }
                });
            } catch (logErr) {
