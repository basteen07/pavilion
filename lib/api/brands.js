...
export async function fetchBrands(options = {}) {
    ...
    let queryStr = 'SELECT * FROM brands WHERE is_active = true';
    const params = [];

    if (categoryId || subCategoryId) {
        // Filter by hierarchy:
        // 1. Brands assigned to this specific Sub-Category
        // 2. Brands assigned to this Category (and no specific Sub-Category)
        // 3. Global Brands (no Category or Sub-Category assigned)

        queryStr += ' AND (';
        const conditions = [];

        // Global brands
        conditions.push('(category_id IS NULL AND sub_category_id IS NULL)');

        if (categoryId) {
            conditions.push(`(category_id = $${params.length + 1} AND sub_category_id IS NULL)`);
            params.push(categoryId);
        }

        if (subCategoryId) {
            conditions.push(`(sub_category_id = $${params.length + 1})`);
